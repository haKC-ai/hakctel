...
static const TFTThemeDef *runtimeTheme = nullptr;
...
{
    return runtimeTheme ? *runtimeTheme : kThemes[resolveThemeIndex()];
}

void setRuntimeTheme(const TFTThemeDef *theme)
{
    runtimeTheme = theme;
    loadThemeDefaults();
}
...
#else
    return getActiveTheme().roles[static_cast<size_t>(TFTColorRole::HeaderBackground)].onColor;
#endif
#else
...
#else
    return getActiveTheme().roles[static_cast<size_t>(TFTColorRole::HeaderTitle)].offColor;
#endif
#else
...
#else
    return getActiveTheme().roles[static_cast<size_t>(TFTColorRole::HeaderStatus)].offColor;
#endif
#else
    return TFTPalette::White;
#endif
...
{
#if GRAPHICS_TFT_COLORING_ENABLED
    return getActiveTheme().roles[static_cast<size_t>(TFTColorRole::FrameMono)].onColor;
#else
...
{
#if GRAPHICS_TFT_COLORING_ENABLED
    return getActiveTheme().roles[static_cast<size_t>(TFTColorRole::FrameMono)].offColor;
#else
    return TFTPalette::White;
#endif
}
...
{
#if GRAPHICS_TFT_COLORING_ENABLED
    return getActiveTheme().fullFrameInvert;
#else
    return false;
#endif
}
...
void setTFTColorRole(TFTColorRole role, uint16_t onColor, uint16_t offColor)
{
    ...
    if (runtimeTheme) {
        roleColors[index].onColorBe = toBe565(runtimeTheme->roles[index].onColor);
        roleColors[index].offColorBe = toBe565(runtimeTheme->roles[index].offColor);
        return;
    }
