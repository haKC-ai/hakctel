...
#ifdef HAKCTEL_ENABLE_PTT
    if (next_key == 30 && modifierFlag == 0 && hakctel::pttEnabled() && audioModule) {
        audioModule->setPttPressed(true);
    }
#endif
...
#ifdef HAKCTEL_ENABLE_PTT
    if (last_key == 30 && modifierFlag == 0 && hakctel::pttEnabled() && audioModule) {
        audioModule->setPttPressed(false);
        return;
    }
#endif
