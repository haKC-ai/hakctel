...
#include "Router.h"
#include "gps/RTC.h"
#ifdef HAKCTEL_ENABLE_PTT
#include "hakctel/HakcTelTheme.h"
#endif
...
#ifdef HAKCTEL_ENABLE_PTT
    if (hakctel::pttEnabled()) {
        moduleConfig.audio.codec2_enabled = true;
        moduleConfig.audio.i2s_ws = DAC_I2S_WS;
        moduleConfig.audio.i2s_sd = DAC_I2S_DIN;
...
void AudioModule::setPttPressed(bool pressed)
{
    pttRequested = pressed;
}
...
int32_t AudioModule::runOnce()
{
    if ((moduleConfig.audio.codec2_enabled) && (myRegion->profile->audioPermitted)) {
        ...
        if (firstTime) {
            ...
            // The pager keyboard drives PTT directly. A configured GPIO remains
            // available for other boards and external hand microphones.
#ifdef HAKCTEL_ENABLE_PTT
            if (moduleConfig.audio.ptt_pin) {
                LOG_INFO("Init external PTT on Pin %u", moduleConfig.audio.ptt_pin);
                pinMode(moduleConfig.audio.ptt_pin, INPUT);
            }
#else
...
#ifdef HAKCTEL_ENABLE_PTT
            pttPressed = pttRequested || (moduleConfig.audio.ptt_pin && digitalRead(moduleConfig.audio.ptt_pin) == HIGH);
            if (radio_state == RadioState::tx &&
                millis() - pttStartedAt >= hakctel::pttMaxSeconds() * 1000UL) {
                pttRequested = false;
                pttPressed = false;
                LOG_WARN("PTT timeout, returning to receive");
            }
#else
            pttPressed = digitalRead(moduleConfig.audio.ptt_pin ? moduleConfig.audio.ptt_pin : PTT_PIN) == HIGH;
#endif
            if (pttPressed) {
